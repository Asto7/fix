...
# local modules
# from video import cv2.VideoCapture
...
def draw_rects(img, rects, color):
    ...
    for x1, y1, x2, y2 in rects:
        ...
        img[int(y1+3):int(y1+h-h/5), int(x1+3):int(x1+w-6)],c = f.preprocessImage(crop_img)
...
if __name__ == '__main__':
    ...
    '''
    # cam = cv2.VideoCapture(video_src, fallback='synth:bg=../data/lena.jpg:noise=0.05')
